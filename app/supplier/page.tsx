...
import { CheckCircle2, Clock, Crown, FileText, MessageCircle, Percent, Sparkles, Star, TrendingUp, Wand2 } from "lucide-react";
...
// (0 = Instap/Starter/Groei, 1 = Pro, 2 = Premium) i.p.v. een los
// bijgehouden lijstje dat uit sync kan raken met de echte tier-gating.
// "Proactieve signalen" was hier ooit een Enterprise-exclusieve zevende
// kaart; sinds Enterprise verwijderd is (aug. 2026) heeft geen enkel
// betaald niveau die perk meer (zie de toelichting in
// app/api/cron/supplier-proactive-signals/route.ts) — de kaart is daarom
// weggehaald i.p.v. iets te beloven dat niemand meer krijgt.
const VYRAI_FEATURES: { icon: ReactNode; title: string; description: string; minTier: "Pro" | "Premium" }[] = [
    ...
    <>
      ...
      <main>
        ...
        {/* "VyrAI zichtbaar maken op /supplier" (livegang-audit): tot nu toe
            ...
            pakket van assistent-functies die vanaf Pro/Premium
            daadwerkelijk meekomen (spec-items #90-98). */}
            ...
            {VYRAI_FEATURES.map((f) => (
              <div key={f.title} className="rounded-2xl border border-line-soft bg-white p-5">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Badge tone={f.minTier === "Premium" ? "clay" : "ochre"}>Vanaf {f.minTier}</Badge>
                </div>
